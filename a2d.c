 ...
 */
 int prefix_sum(int nthreads, int sequenceType){
     ...
     pthread_mutex_lock(&state_arr[0].lock);
     state_arr[0].sum = state_arr[0].value;
     pthread_cond_signal(&state_arr[0].cv);
     pthread_mutex_unlock(&state_arr[0].lock);
 ...
 void *doWork(void *a){
     ...
     thread_function_args * args = malloc(sizeof(thread_function_args));
     args=(thread_function_args *)a;

     pthread_mutex_lock(&args->state_arr[args->s_index].lock);

     while((args->state_arr[args->s_index-1].sum) == 0)
     {
         pthread_cond_wait(&args->state_arr[args->s_index-1].cv, &args->state_arr[args->s_index].lock);
     }

     args->state_arr[args->s_index].sum = args->state_arr[args->s_index-1].sum +
             args->state_arr[args->s_index].value;

     pthread_cond_signal(&args->state_arr[args->s_index].cv);

     pthread_mutex_unlock(&args->state_arr[args->s_index-1].lock);
